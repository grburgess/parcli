...
- `ui::theme` palette defines carrier-badge colors (FNV-based per-carrier hash, stable across runs) plus the ACCENT/DIM/WARN/ZEBRA/ERROR_BG accents, and `key_hint` for footer hints — the key in bold ACCENT, the label in DIM, uniformly for every hint (not tinted by mode or urgency). `status_color` (status → color) predates the theme module and still lives in `ui/mod.rs`, not `ui::theme`.
- `journey` module: `journey_stops` extracts origin, waypoints, current, and home from `TrackEvent` locations (deduped case-insensitively); `render_strip` formats the visual journey line (● origin ━━ waypoints ━━ ◉ current ┄┄ ○ home, `✔` at home once delivered); a pulse (`▸`) advances one dashed segment per render tick (250 ms) toward home; a Delivered parcel renders fully solid with no pulse.
- Map pane: `Canvas` widget renders a built-in world `Map`; `map_bounds` pads the stops' bounding box 15% on each side with a minimum span of 10° longitude x 6° latitude, clamped to valid lon/lat ranges. Map pins are `●` origin, `•` waypoint, `◉`/`○` current (alternating each tick), `⌂` home — the journey strip instead uses `○` for home (`✔` once delivered). Pin labels flip to left-align for stops in the eastern half of the current bounds, so they don't clip the canvas's right edge. The `m` key toggles `show_map`; the pane is drawn in detail view only when `show_map` is true and the outer area is ≥ 100 columns wide.
- Geocoding via OpenStreetMap Nominatim: `featureType=settlement` for event locations, full address for home; ≥1.1 s throttle between requests (the throttle timestamp is set before the request is sent, so a timed-out request still counts); event-location misses are cached as `null` in `StateCache.geo` to avoid repeated failed lookups, keyed by the lowercased normalized place name. A home-address miss is deliberately *not* cached, so it is retried on every startup rather than getting stuck unresolved.
- `PollEvent::Geocoded { key, coord }` reports one geocode result to the UI (a lowercased normalized place, or the raw home address, mapped to `Some(coord)`/`None`) — it does not enqueue geocoding work; the poller does the geocoding itself and sends this only once a result is known.
- `ParcelList.home: Option<String>` persisted in `parcels.toml`; `--home <ADDRESS>` sets it and saves immediately; `--home ""` (empty or whitespace) clears it instead.
- Header home indicator (`ui::draw_header`): `⌂ set` (green) when `cache.geo.get(home)` is `Some(Some(_))`, `⌂ unresolved` (WARN) when it is `Some(None)`, `⌂ locating…` (DIM) while the key is absent from the cache, `⌂ unset` (DIM) when no home is configured.
